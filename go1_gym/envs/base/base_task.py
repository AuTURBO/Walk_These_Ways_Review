...
class BaseTask(gym.Env):
    ...
    def handle_keyboard_events(self):
        if self.viewer:  # Ensure the viewer exists
            for evt in self.gym.query_viewer_action_events(self.viewer):
                if evt.action == "forward" and evt.value > 0:
                    self.commands[:, 0] += 0.5
                    print(f"forward velocity increased to {self.commands[:, 0]}")
                
                if evt.action == "backward" and evt.value > 0:
                    self.commands[:, 0] -= 0.5
                    print(f"forward velocity decreased to {self.commands[:, 0]}")
                    
                if evt.action == "turn_right" and evt.value > 0:
                    self.commands[:, 2] -= 0.3
                    print(f"angular velocity increased to {self.commands[:, 2]}")
                    
                if evt.action == "turn_left" and evt.value > 0:
                    self.commands[:, 2] += 0.3
                    print(f"angular velocity decreased to {self.commands[:, 2]}")
