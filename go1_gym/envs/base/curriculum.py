...
class Curriculum:
    ...
    def __init__(self, seed, **key_ranges):
        ...
        #print(f"size",np.size( *cfg.values()))
        #print(f"cfgval:",*cfg.values())
        #print(f"cfg ty",type(*cfg))
        ...
        #print(f"len keys", len(self.keys))
    ...
    def sample_uniform_from_cell(self, centroids):
        ...
        # Calculate low and high bounds for each bin based on its centroid
        low_bounds = centroids - bin_sizes / 2
        high_bounds = centroids + bin_sizes / 2
        
        # Print the range of each bin
        for low, high in zip(low_bounds, high_bounds):
            print(f"Bin range: {low} to {high}")
        
        # Sample uniformly from each cell
        sampled_values = self.rng.uniform(low_bounds, high_bounds)
        #for i, value in enumerate(sampled_values):
        #    #print(f"Sampled value from bin {i}: {value}")
...
class RewardThresholdCurriculum(Curriculum):
    ...
    def update(self, bin_inds, task_rewards, success_thresholds, local_range=0.5):
        ...
        for adjacent in adjacents:
            ...
            print(f"adj update: {adjacent_inds}", self.weights[adjacent_inds])
    ...
    def print_all_bin_ranges_for_param(self, param_name):
        """
        Print all bin ranges for a specific parameter.
        param_name: str, the name of the parameter (e.g., 'x_vel')
        """
        if param_name not in self.cfg:
            #print(f"Parameter {param_name} not found in curriculum configuration.")
            return
        
        # Retrieve the parameter configuration
        param_cfg = self.cfg[param_name]
        
        # Calculate bin sizes and start points
        bin_size = (param_cfg[-1] - param_cfg[0]) / (len(param_cfg) - 1)
        bin_starts = param_cfg[:-1]
        
        # Print the bin ranges
        #print(f"All bin ranges for {param_name}:")
        for start in bin_starts:
            end = start + bin_size
            #print(f"{start:.3f} to {end:.3f}")
...
if __name__ == '__main__':
    ...
    adjacents = r.get_local_bins(np.array([10, ]), 0.5)
